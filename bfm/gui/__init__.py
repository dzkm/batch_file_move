...
def draw_gui():
    main_window = windows.main.draw()
    styles.load_styles(main_window)
...
def _close_when_empty(response: any) -> any:
    while True:
        ...
        if messagebox.askokcancel(
            "Quit", "You canceled your operation. Do you want to quit?"
        ):
            _root.destroy()
            raise SystemExit(0)
...
def ask_kind_list() -> int:
    ...
    return _close_when_empty(
        simpledialog.askinteger(
            "Type of list", prompt, parent=_root, minvalue=1, maxvalue=999
        )
    )
...
def ask_raw_input() -> str:
    return _close_when_empty(
        simpledialog.askstring("Raw input", "Enter raw input:", parent=_root)
    )
